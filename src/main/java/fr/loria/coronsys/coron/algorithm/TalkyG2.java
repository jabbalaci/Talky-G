 ...
 * Talky-G v2
 * 
 * Like Talky-G, but the another hash (a HashMap) is used. The old
 * hash structure adopted from Charm is eliminated.
 * 
 * Output: FGs (frequent generators). It uses a normal hash.
 *  
 * @author Laszlo Szathmary (<a href="Szathmary.L@gmail.com">Szathmary.L@gmail.com</a>)
 */
public class TalkyG2 
implements Algorithm
{
   ...
   /**
    * Frequent generators are stored in this too. When we find a rare 
    * itemset, we want to check ig it's a minimal rare generator, thus
    * in this hash we lookup all its one-size smaller subsets.
    * By definition, if it's an mRG, all its subsets are FGs.
    * 
    * Add the empty set to it! The empty set is an FG and it's
    * necessary for rare attributes!
    */
   private Map<BitSet, Integer> fgMap = new HashMap<BitSet, Integer>();
    ...
    */
   public TalkyG2(Vector<BitSet> database, int min_supp)
   {
      ...
      //
      // the empty set is an FG; by def. it's support is 100%
      fgMap.put(new BitSet(), Database.getNumberOfObjects());
   }
   ...
   public void start()
   {
      ...
      //this.fgMap = null;   // not needed anymore
   ...
   private ITnode getNextGenerator(ITnode curr, ITnode other)
   {
      ...
      //   return null;
      //}
      if (this.candSubsumesAnFgOrCandHasANonFgSubset(cand))
      {
         //System.out.println(candidate.toStringName() + " [>has an FG subset<]");
         return null;
      }
   ...
   /**
    * Here we test the FG status of the candidate. When we get here, the candidate
    * is frequent and its tidset is different from its parents.
    * Now, let's see if cand subsumes an already found FG. Subsuming means that
    * cand has a proper FG subset with the same support. This means that cand
    * is not an FG.
    * To that end, we look up all one-size smaller subsets of cand and check 
    * if cand subsumes a subset.
    * However, it can be possible that a subset is missing, i.e. it's not present
    * in the hash. This means that the subset is not FG. In this case cand
    * is not an FG either.
    * 
    * So, if cand fails the FG test, we return true.
    * If cand is an FG, we return false.
    * 
    * @param cand An FG candidate.
    * @return True if cand fails the FG test. False, otherwise.
    */
   private boolean candSubsumesAnFgOrCandHasANonFgSubset(ITnode cand)
   {
      Vector<BitSet> subsets = SetOp.getOneSizeSmallerSubsetsOf(cand.getIntent());
      int cand_supp = cand.getSupp();
      BitSet ss;    // subset
      Integer stored;
      
      for (Enumeration<BitSet> e = subsets.elements(); e.hasMoreElements(); )
      {
         ss = e.nextElement();
         stored = this.fgMap.get(ss);
         if (stored != null)
         {
            if (stored == cand_supp) {
               return true;
         ...
         {
            /*
             * We want to check if cand is an FG. If it's an FG, then all its subsets are FGs.
             * We only check the one-size smaller subsets. If stored == null, then a one-size
             * smaller subset of cand is not an FG, thus cand cannot be an FG either.
             */
            //System.out.println(">>> subset missing; cand: "+cand.getIntent()+", missing subset: "+ss);
            return true;
         }
      }
   ...
   {      
      ...
      this.fgMap.put(node.getIntent(), node.getSupp());
      ++this.fgCnt;
