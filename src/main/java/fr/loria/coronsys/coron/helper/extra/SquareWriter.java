...
/**
 * This class is responsible for converting the dataset in square format.
 * Square format: this is an RCF file that contains information about
...
public class SquareWriter
{
    ...
    */
   public SquareWriter(Vector<BitSet> database)
   {
   ...
   private void printExtraInfo()
   {
      ...
      sb.append("# Output DB size: ")
        .append("(").append(m).append(" * ").append(m-1).append(")")
        .append(" x ")
        .append("(").append(n).append(" * ").append("3").append(")")
        .append(" = ").append(m * (m-1)).append(" * ").append(n * 3)
        .append("\n");
   ...
   {      
      ...
      {                                   
         for (j = 1; j <= size; ++j)
         {
            ...
            set1 = (BitSet) this.database.get(i);
            set2 = (BitSet) this.database.get(j);
